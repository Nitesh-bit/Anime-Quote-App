...
const getDetails = (q) => {
  ...
  animeCharacter.innerHTML = q.character;
  animeQuote.innerHTML = `"${q.quote}"`;
};
...
const getQuotebyName = (name) => {
  fetch(`https://animechan.vercel.app/api/random/anime?title=${name}`)
    .then((response) => {
      if (response.ok) {
        return response.json();
      } else {
        alert("Please enter correct anime name");
        throw new Error(response);
      }
    })
    .then((q) => {
...
submit1.addEventListener("click", (e) => {
  e.preventDefault();
  getQuotebyName(quote.value);
});

const getQuotebyCharacter = (character) => {
  fetch(`https://animechan.vercel.app/api/random/character?name=${character}`)
    .then((response) => {
      if (response.ok) {
        return response.json();
      } else {
        alert("Please enter correct anime character");
        throw new Error(response);
      }
    })
    .then((q) => {
...
submit2.addEventListener("click", (e) => {
  e.preventDefault();
  getQuotebyCharacter(quote2.value);
});
